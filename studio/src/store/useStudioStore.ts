...
export type RouteName = 'home' | 'studio' | 'sop' | 'share' | 'player' | 'brand' | 'library' | 'shared' | 'recent' | 'templates' | 'knowledge' | 'team' | 'analytics' | 'docs' | 'shared-doc' | 'privacy';
    ...
    try {
      ...
      if (Array.isArray(sessionData.steps)) {
        sessionData.steps = sessionData.steps.map((s: any, i: number) => ({
          ...
          action: s.action || s.data?.action || s.type || 'click',
          ...
          url: s.url || s.data?.frameUrl || s.data?.url || '',
